...
//
// IDanteBuffers.hpp
// Shim to decouple Buffer accessors from Dante::Buffers
//
...
public:
	...
	/** Check if connected to the underlying shared memory buffer(s) */
	virtual bool isConnected() const = 0;

	/** Get a pointer to the shared memory metadata block */
	virtual const volatile Dante::buffer_header_t * getBufferHeader() const = 0;

	/** Get a pointer to the shared memory timing object subheader */
	virtual const Dante::timing_object_subheader_t * getTimingObjectSubheader() const = 0;
...
class DanteBuffersAdapter : public Dante::IDanteBuffers
{
public:
	DanteBuffersAdapter(Dante::Buffers & buffers)
		: mBuffers(buffers)
	{}

	bool isConnected() const override { return mBuffers.isConnected(); }

	const volatile Dante::buffer_header_t * getBufferHeader() const override { return mBuffers.getHeader(); }

	const Dante::timing_object_subheader_t * getTimingObjectSubheader() const override
	{
		return mBuffers.getTimingObjectSubheader();
	}

	void * getDanteTxChannel(unsigned int index) const override { return mBuffers.getDanteTxChannel(index); }

	void * getDanteRxChannel(unsigned int index) const override { return mBuffers.getDanteRxChannel(index); }
